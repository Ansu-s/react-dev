...
export class NewsArticles extends Component {
    ...
    async componentDidMount(){
        let apiUrl = "https://newsapi.org/v2/top-headlines?sources=techcrunch&apiKey=dca7e9c409944d43aee58da28e78627a";
    ...
    render() {
        return (
            <div className="container my-3">
                <h2>Here are the todays headlines... :)</h2>
                <div className="row"> 
                {this.state.articles.map((element)=>{
                    return <div className="col-md-4" key={element.url}>
                        <ArticleItems title={element.title?element.title:""} description={element.description?element.description:""} imageUrl={element.urlToImage} newsUrl={element.url}/>
                    </div> 
                })} 
                </div> 
            </div>
